...
describe("normalized hook handling", () => {
  // 2119: REQ-004.1.3
  it("extracts edited paths from file_path fields and Codex apply_patch payloads", () => {
  ...
  // 2119: REQ-004.1.3
  it("ignores edits to files outside the spec and test globs", () => {
    const root = fixture(BAD_SPEC);
    writeFileSync(join(root, "app.py"), "print('hi')\n");
    const out = handleHook(root, "after-edit", "claude", { tool_input: { file_path: join(root, "app.py") } });
    expect(out).toEqual({});
  });

  // 2119: REQ-004.1.4
  it("injects lint violations for an edited spec file as additional context", () => {
    const root = fixture(BAD_SPEC);
    const out = handleHook(root, "after-edit", "claude", {
    ...
    const gem = handleHook(root, "after-edit", "gemini", {
      tool_input: { file_path: join(root, "specs/REQ-001-widgets.md") },
  ...
  // 2119: REQ-004.1.5
  it("blocks stop with a reason and fix commands while check is failing", () => {
    const root = fixture(GOOD_SPEC, { annotate: false }); // uncovered MUST
    const out = handleHook(root, "stop", "claude", {}) as { decision?: string; reason?: string };
    expect(out.decision).toBe("block");
    expect(out.reason).toContain("REQ-001.1.1");
    expect(out.reason).toContain("npx rfc2119 check");
    const gem = handleHook(root, "stop", "gemini", {}) as { decision?: string };
    expect(gem.decision).toBe("deny");
  });

  // 2119: REQ-004.1.6
  it("returns a non-blocking response when stop_hook_active is set", () => {
    const root = fixture(GOOD_SPEC, { annotate: false });
    expect(handleHook(root, "stop", "claude", { stop_hook_active: true })).toEqual({});
  });

  // 2119: REQ-004.1.7
  it("injects workflow context on session-start", () => {
    const root = fixture();
    const out = handleHook(root, "session-start", "claude", {}) as {
  ...
  // 2119: REQ-004.1.8
  it("is a silent no-op in repositories where 2119 is not set up", () => {
    const root = mkdtempSync(join(tmpdir(), "2119-empty-"));
    for (const event of ["after-edit", "stop", "session-start"] as const) {
      expect(handleHook(root, event, "claude", {})).toEqual({});
    }
  });

  // 2119: REQ-004.1.9
  it("reports handler errors inside the JSON response instead of throwing", () => {
    const root = mkdtempSync(join(tmpdir(), "2119-bad-"));
    writeFileSync(join(root, ".2119.yml"), "specs: {not: [valid, shape\n");
    const out = handleHook(root, "stop", "claude", {}) as { systemMessage?: string };
