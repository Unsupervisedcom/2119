...
export const AGENT_WORKFLOW_TOPICS = {
  planning: `**When planning a feature**, write or update a spec in \`specs/\` first. Every
...
each. A flawed requirement steers the whole implementation wrong.`,
  classification: `**Classify before making it normative**: use requirements for observable product
...
proof obligations.`,
  granularity: `**Requirement granularity**: A first-pass feature spec should aim for around
...
implementation details rather than making every detail an enforced \`MUST\`.`,
  implementation: `**When implementing**, every MUST/SHALL requirement needs at least one test
...
rather than duplicating the test for traceability.`,
  reviewerDiversity: `**Reviewer diversity**: use reviewer models from different providers, routinely
...
requirements; a single model family shares blind spots.`,
  gate: `**Before finishing any task**, run \`npx rfc2119 check\`. It must exit 0. If it
...
${Object.entries(AGENT_WORKFLOW_TOPICS)
  .map(([id, text]) => `<!-- 2119-workflow:${id} -->\n${text}`)
  .join("\n\n")}
<!-- 2119:end -->
`;
