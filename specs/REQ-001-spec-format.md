...
`REQ-NNN-<topic>.md`, where `NNN` is a zero-padded sequence number. An optional
project prefix (e.g. `DW-REQ-NNN`) may be configured.
...
1. An existing requirement's ID MUST NOT be renumbered or reassigned to a different obligation. [review: specs/**]
2. New requirements MUST be appended at the end of their section (or in a new section) with the next sequential number. [review: specs/**]
3. A removed requirement MUST keep its number, with the statement body replaced by the exact text `REQUIREMENT REMOVED`, so that external references (tests, verdicts) never dangle. [review: specs/**]
